...
window.addEventListener("load", () => {
  ...
  if (!bar || !indicator || !tabs.length) return;
  ...
  function moveIndicator(activeTab) {
    ...
    indicator.style.width = Math.max(44, width - 12) + "px";
    indicator.style.transform = `translateY(-50%) translateX(${left + 6}px)`;
  }

  const active = bar.querySelector(".tab-item.active") || tabs[0];
  if (active) moveIndicator(active);

  // animate on tap (doesn't block navigation)
  tabs.forEach(t => {
    t.addEventListener("click", () => moveIndicator(t), { passive: true });
  });

  window.addEventListener("resize", () => {
    const a = bar.querySelector(".tab-item.active") || tabs[0];
